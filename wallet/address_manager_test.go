...
func TestGetUnusedAddress(t *testing.T) {
	...
	deps.store.On("ListTxDetails", mock.Anything, db.ListTxDetailsQuery{
		WalletID:    w.id,
		StartHeight: 0,
		EndHeight:   -1,
	...
	deps.store.On("ListTxDetails", mock.Anything, db.ListTxDetailsQuery{
		...
		MsgTx:        &wire.MsgTx{TxOut: []*wire.TxOut{{PkScript: firstPkScript}}},
...
func TestGetDerivationInfoNoDerivationInfo(t *testing.T) {
	...
	deps.store.On("NewImportedAddress", mock.Anything, db.NewImportedAddressParams{
		WalletID:     w.id,
		...
		t, addr, db.WitnessPubKey, waddrmgr.KeyScopeBIP0084, pubKey,
	), nil).Once()
